...
def stepTrack(mcu, ch, adpKey, cntwin):
  ...
  adpKey.phase = 0
  adpKey.att = 0
  cntwin.addstr(4,10,"Att: %3.1f dB  Phase: %4d iteration: 0 total_step: 0" %((adpKey.att/2), adpKey.phase))
  cntwin.refresh()
  th = threading.Thread(target=mcu.senddata, args=(ch, adpKey.att, adpKey.phase,))
  th.start()
  cntwin.refresh()
...
def step_phase_tune(mcu, ch, adpKey, basePoint, cntwin, debug, param):
  ...
  if param.direction == "increase":
    ...
    if np.sign(min(increase_delta_List)) == -1:  # 最小値がマイナスの符号なら最小値の更新があったと考えられる
      ...
      th.join()
      time.sleep(1)
      param.pv = min(increase_delta_List) - param.pv
      param.cv = param.pv
      param.delta_calc()
      debug.set(adpKey, basePoint, param)
      param.flag = "True"
    ...
    if param.flag == "True":
      ...
      while True:
        ...
        if param.cv < param.pv :
          ...
          cntwin.addstr(4,10,"Att: %3.1f dB  Phase: %4d step_phase: %d total_step: %d" %((basePoint.att/2), basePoint.phase, param.step_phase, param.total_step))
          cntwin.refresh()
    ...
    if np.sign(min(decrease_delta_List)) == -1:  # 最小値がマイナスの符号なら最小値の更新があったと考えられる
      ...
      th.join()
      time.sleep(1)
      param.pv = min(decrease_delta_List) - param.pv
      param.cv = param.pv
      param.delta_calc()
      debug.set(adpKey, basePoint, param)
      param.flag = "True"
...
def step_att_tune(mcu, ch, adpKey, basePoint, cntwin, debug, param):
  ...
  while True:
    ...
    if param.cv < param.pv :
      ...
      cntwin.erase()
      cntwin.addstr(9,5, "step track制御を開始")
      cntwin.addstr(10,5, "attの探索を終了")
      cntwin.addstr(4,10,"Att: %3.1f dB  Phase: %4d step_att: %d total_step: %d" %((basePoint.att/2), basePoint.phase, param.step_att, param.total_step))
      cntwin.refresh()
      th = threading.Thread(target=mcu.senddata, args=(ch, basePoint.att, basePoint.phase,))
      th.start()
      th.join()
